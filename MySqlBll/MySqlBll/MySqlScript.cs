...
namespace MySqlBll
{
	public class MySqlScript
	{
		...
		public List<string> MakeScript()
		{
			...
			using (List<TABLE>.Enumerator enumerator = this.m_Source.TABLES.GetEnumerator())
			{
				while (enumerator.MoveNext())
				{
					TABLE sourceTable = enumerator.Current;
					...
					else
					{
						...
						string temp = this.MakeChange(sourceTable, targetTable);
						if (temp != "")
						{
							list.Add(temp);
						}
		...
		private string MakeChange(TABLE sourceTable, TABLE targetTable)
		{
			string change = "";
			...
			string primary = "";
			using (List<COLUMN>.Enumerator enumerator = sourceTable.COLUMNS.GetEnumerator())
			{
				while (enumerator.MoveNext())
				{
					COLUMN sourceColumn = enumerator.Current;
					...
					else if (!this.EqualsColumn(sourceColumn, targetColumn2))
					{
						string text = change;
						change = string.Concat(new string[]
						{
			...
			using (List<COLUMN>.Enumerator enumerator = targetTable.COLUMNS.GetEnumerator())
			{
				while (enumerator.MoveNext())
				{
					COLUMN targetColumn = enumerator.Current;
		...
		private bool EqualsColumn(COLUMN source, COLUMN target)
		{
			bool result;
